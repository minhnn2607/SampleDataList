...
class ResultObservable<T>(private val upstream: Observable<Response<T>>) :
    Observable<Result<T>?>() {
    override fun subscribeActual(observer: Observer<in Result<T>?>) {
        upstream.subscribe(ResultObserver(observer))
    }

    private class ResultObserver<R> constructor(private val observer: Observer<in Result<R>?>) :
        Observer<Response<R>?> {
